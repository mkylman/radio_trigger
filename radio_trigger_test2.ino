...
byte aRead() {
  ...
  return (ADCH >> SQUASH); // smash it way down
}
...
int main(void) {
  ...
  ADMUX |= 0b01100000;   // REFS1 to 0, REFS0 to 1: set VCC as reference voltage
                         // ADLAR to 1 - left justify ADC so can just read 8-bit ADCH
                         // select ADC0
